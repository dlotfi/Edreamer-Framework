...
namespace Edreamer.Framework.Media.Storage
{
    public class FileSystemStorageProvider : IStorageProvider
    {
        ...
        public FileSystemStorageProvider(ISettingsService settingsService)
        {
            var storageVirtualPath = settingsService.GetStoragePath();
            _storagePath = HostingEnvironment.IsHosted
                                ? HostingEnvironment.MapPath(storageVirtualPath) ?? ""
                                : Path.Combine(AppDomain.CurrentDomain.BaseDirectory, storageVirtualPath.TrimStart("~/"));
            ...
            _publicPath = appPath + storageVirtualPath.TrimStart("~/");
            if (!_publicPath.EndsWith("/")) _publicPath = _publicPath + "/";
        }
        ...
        {
            var mappedPath = string.IsNullOrEmpty(path) ? _storagePath : Path.Combine(_storagePath, path);
        ...
        public IStorageFile GetFile(string path)
        {
            ...
            Throw.IfNot(fileInfo.Exists)
                .AnArgumentException("File {0} does not exist".FormatWith(path));
        ...
        public IEnumerable<IStorageFile> ListFiles(string path)
        {
            ...
            Throw.IfNot(directoryInfo.Exists)
                .AnArgumentException("Directory {0} does not exist".FormatWith(path));
        ...
        public IEnumerable<IStorageFolder> ListFolders(string path)
        {
            ...
            if (!directoryInfo.Exists)
            {
                ...
                {
                    Throw.Now.A<ArgumentException>("The folder could not be created at path: {0}. {1}".FormatWith(path, ex), ex);
                }
            }
            return directoryInfo
                .GetDirectories()
        ...
        {
            var directoryInfo = new DirectoryInfo(MapStorage(path));
            Throw.If(directoryInfo.Exists)
                .AnArgumentException("Directory {0} already exists".FormatWith(path));
        ...
        public void RenameFolder(string oldPath, string newPath)
        {
            ...
            Throw.IfNot(sourceDirectory.Exists)
                .AnArgumentException("Directory {0} does not exist".FormatWith(oldPath));

            var targetDirectory = new DirectoryInfo(MapStorage(newPath));
            Throw.If(targetDirectory.Exists)
                .AnArgumentException("Directory {0} already exists".FormatWith(newPath));
        ...
        public void RenameFile(string oldPath, string newPath)
        {
            ...
            Throw.IfNot(sourceFileInfo.Exists)
                .AnArgumentException("File {0} does not exist".FormatWith(oldPath));

            var targetFileInfo = new FileInfo(MapStorage(newPath));
            Throw.If(targetFileInfo.Exists)
                .AnArgumentException("File {0} already exists".FormatWith(newPath));
        ...
        {
            var fileInfo = new FileInfo(MapStorage(path));
            Throw.If(fileInfo.Exists)
                .AnArgumentException("File {0} already exists".FormatWith(fileInfo.Name));
        ...
        private static string ValidatePath(string basePath, string mappedPath)
        {
            ...
            Throw.IfNot(valid).AnArgumentException("Invalid path");
        ...
        private class FileSystemStorageFile : IStorageFile
        {
            ...
            public FileSystemStorageFile(string path, FileInfo fileInfo)
            {
                ...
                Throw.IfArgumentNull(fileInfo, "fileInfo");
                _path = path;
                _fileInfo = fileInfo;
            }
        ...
        private class FileSystemStorageFolder : IStorageFolder
        {
            ...
            {
                Throw.IfArgumentNull(path, "path");
                Throw.IfArgumentNull(directoryInfo, "directoryInfo");
                _path = path;
            ...
            {
                Throw.IfNull(_directoryInfo.Parent)
                    .AnArgumentException("Directory {0} does not have a parent directory".FormatWith(_directoryInfo.Name));
