...
namespace Edreamer.Framework.Security.Encryption
{
    public class EncryptionService : IEncryptionService
    {
        ...
        public EncryptionService(ISettingsService settingsService)
        {
        ...
        {
            var algorithm = SymmetricAlgorithm.Create(_settingsService.GetEncryptionSettings().EncryptionAlgorithm);
            algorithm.Key = _settingsService.GetEncryptionSettings().EncryptionKey.ToByteArray();
            return algorithm;
        }

        private HMAC CreateHashAlgorithm()
        {
            var algorithm = HMAC.Create(_settingsService.GetEncryptionSettings().HashAlgorithm);
            algorithm.Key = _settingsService.GetEncryptionSettings().HashKey.ToByteArray();
            return algorithm;
        }
